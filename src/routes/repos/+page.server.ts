...
export const load: PageServerLoad = async ({ url }) => {
	return withSpan(
		'load:repos',
		{
			'code.filepath': 'src/routes/repos/+page.server.ts',
			'http.route': '/repos'
		},
		async () => {
			...
			const sortKey: SortKey =
				sortParam && validSortKeys.includes(sortParam) ? sortParam : 'activity';
			const sortDir = url.searchParams.get('dir') === 'asc' ? 'asc' : 'desc';

			// Health severity: higher = worse. Default desc = worst first.
			const healthSeverity: Record<HealthStatus, number> = {
				green: 0,
				yellow: 1,
				red: 2
			};
